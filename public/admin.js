  ...
  try {
    ...
    function renderUsers(users) {
      ...
      users.forEach((u) => {
        ...
        tdDisplay.textContent = u.displayName || "";
    ...
    async function loadImpersonatePage(p = 1) {
      ...
      try {
        const resp = await fetch(
          `/admin/impersonate?page=${page}&pageSize=${pageSize}`,
        );
        if (!resp.ok) {
    ...
    if (prevBtn)
      prevBtn.addEventListener("click", () => {
        if (page > 1) loadImpersonatePage(page - 1);
      });
    if (nextBtn)
      nextBtn.addEventListener("click", () => {
        loadImpersonatePage(page + 1);
      });
    if (pageSizeSelect)
      pageSizeSelect.addEventListener("change", () => loadImpersonatePage(1));
