  ...
  const { needsBackup, backupBlobKey, lastBackupHash, lastBackupBlobKey } =
    await db.transaction(async (tx) => {
      ...
      const lastBackup = await tx
        ...
        .from(schema.databaseBackups)
        .where(
          eq(schema.databaseBackups.timestamp, lastBackupTs[0]?.timestamp ?? 0),
        );
...
app.post("/signup", async (req, res) => {
  ...
  if (!displayName || !username || !password) {
    return res
      .status(400)
      .json({ error: "Display name, username, and password are required" });
  }
...
app.get("/progress", requireAuth, async (req, res) => {
  ...
  const set = (
    path: string[],
    val: any,
    ptr: Record<string, any> = progressResult,
  ) => {
    if (path.length === 1) {
      ptr[path[0]!] = val;
      return;
...
app.patch("/progress", requireAuth, async (req, res) => {
  const { cambridgeVersion, partName, testName, result, needReview } =
    req.body || {};
  const userId = req.session.userId!;

  await db.transaction(async (tx) => {
    const updateResult = await tx
      .update(schema.camProgresses)
...
app.post("/feelings", requireAuth, async (req, res) => {
  ...
  if (!timestamp || !name || !feeling) {
    return res
      .status(400)
      .json({ error: "timestamp, name, and feeling are required" });
  }
...
app.patch("/feelings/:id", requireAuth, async (req, res) => {
  ...
  if (Object.keys(update).length === 0) {
    return res
      .status(400)
      .json({ error: "At least one of feeling or isLoved must be provided" });
  }
...
app.patch("/todos/:id", requireAuth, async (req, res) => {
  ...
  if (!text && completed === undefined) {
    return res
      .status(400)
      .json({ error: "Text or completed status is required" });
  }
